...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    data = items[['order_id','product_id','freight_value']].join(products[['product_id','product_weight_g']].set_index('product_id'),
                                                             on='product_id',
                                                             how='inner')\
                                                           .join(orders[['order_id', 'order_status']].set_index('order_id'),
                                                             on='order_id',
                                                             how='inner')
    ...
    aggregations = delivered.groupby(by='order_id').agg({'freight_value':'sum', 'product_weight_g':'sum'}).reset_index()
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    order_purchase_ammount_per_date = filtered_dates.groupby(by='order_purchase_date').agg({'order_purchase_date':'count'})\
                                                                                      .reset_index(names='date')\
                                                                                      .rename(columns={'order_purchase_date':'order_count'})
    ...
    result_df = order_purchase_ammount_per_date[['order_count','date']]
